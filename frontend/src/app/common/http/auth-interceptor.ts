...
export class AuthInterceptor implements HttpInterceptor {
  ...
  private reAuth(): Observable<HttpResponse<TokenDTO>> {
    return this.api
      .post<TokenDTO>('/api/auth/refresh', {
        token: this.sessionService.getRefreshToken(),
      })
      .pipe(
        tap((x) => {
          if (!x.body?.accessToken) {
            // 👺 ???????? HttpClient why no throw proper error ??????????????????????? 😥🙈👿
            this.sessionService.clearAccessToken();
            this.sessionService.clearRefreshToken();
            of(this.router.navigateByUrl('/login'));
          }
